					...
					if(cmd.replaceFirst("modprobe ","").startsWith("-")){
						if(tmpStr.startsWith("-r")){
							...
							if(msh.enabled.containsKey(tmpStr.split(" ")[1])){
								...
								System.out.println("Module \""+tmpStr.split(" ")[1]+"\" doesn't enabled or loaded");
							}
...
//						tmp=(module) m.newInstance();
				...
				}else if(cmd.contains(":")) if(cmd.split(" ")[0].contains(":")) if(msh.enabled.containsKey(cmd.split(":")[0])) if(cmd.split(" ")[0].split(":").length==2){
//					System.out.println(cmd.split(" ")[0].split(":").length);
